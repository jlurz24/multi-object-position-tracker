...
class PositionVelocityMeasurer {
 ...
 private:
    ...
    ros::ServiceClient worldPropsServ = nh.serviceClient<gazebo_msgs::GetWorldProperties>("/gazebo/get_world_properties");
    ...
    do {
      ...
      for(unsigned int i = 0; i < currentOrder.size(); ++i){
         if(i >= objects->positions.size()){
          ...
          positionDeviation += sqrt(square(knownPosition.x - estimatedPosition.x) + square(knownPosition.y - estimatedPosition.y));

          const geometry_msgs::Vector3& knownVelocity = knownVelocities[currentOrder[i]].linear;
          const geometry_msgs::Vector3& estimatedVelocity = objects->velocities[i].twist.linear;

          velocityDeviation += sqrt(square(knownVelocity.x - estimatedVelocity.x) + square(knownVelocity.y - estimatedVelocity.y));
      }

      if(positionDeviation + velocityDeviation < currPositionDeviation + currVelocityDeviation){
   ...
   ROS_INFO("Current Position Deviation: %f, Current Velocity Deviation: %f", currPositionDeviation, currVelocityDeviation);
